...
describe Crawlberg do
  describe "links" do
    ...
    it "Resolves relative URLs using base tag href" do
      ...
      __result.links.any? { |__el| __el.url.to_s.downcase.includes?("example.com") }.should be_true
    end
    ...
    it "Handles empty href attributes without errors" do
      ...
      __result.links.any? { |__el| __el.url.to_s.downcase.includes?("/valid") }.should be_true
    end
    ...
    it "Skips mailto:, javascript:, and tel: scheme links" do
      ...
      __result = Crawlberg.scrape(engine, url)
      (__result.links.size || 0).should be > 0
      __result.links.all? { |__el| !__el.url.to_s.downcase.includes?("mailto:") }.should be_true
    end
    it "Handles protocol-relative URLs (//example.com) correctly" do
      ...
      __result.links.any? { |__el| __el.url.to_s.downcase.includes?("//") }.should be_true
    end
